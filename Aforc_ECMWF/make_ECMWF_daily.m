...
if Download_data==1
  ...
  download_ECMWF_daily(Ymin,Ymax,Mmin,Mmax,lonmin,lonmax,latmin,latmax,...
   ...
   for Y=Ymin:Ymax
    ...
    if Y==Ymax
     ...
     for M=mo_min:mo_max
      ...
      if exist(fname)==0
	    ...
	    if makefrc==1
          for aa=1:itolap_ecmwf
	     nc_frc{'sms_time'}(aa)=nc{'time'}(tndx-(itolap_ecmwf-aa));
          end
        end
	    %
        if makeblk==1
          for aa=1:itolap_ecmwf
	      nc_blk{'bulk_time'}(aa)=nc{'time'}(tndx-(itolap_ecmwf-aa));
	  end
        end
	    close(nc)
      end
      ...
      for tndx=1:tlen0
	    ...
	    interp_ECMWF(ECMWF_dir,Y,M,Roa,interp_method,lon1,lat1,...
		    mask,tndx,nc_frc,nc_blk,lon,lat,angle,tndx+itolap_ecmwf)	
      end
	...
	if makefrc==1
	   ...
	   for tndx=tlen0+itolap_ecmwf+1:tlen;
	      nc_frc{'sms_time'}(tndx)=nc{'time'}(tndx-tlen0-(itolap_ecmwf));
	   end;
	end
	%
        if makeblk==1
	   disp('bulk_time')
           for tndx=tlen0+itolap_ecmwf+1:tlen;
              nc_blk{'bulk_time'}(tndx)=nc{'time'}(tndx-tlen0-(itolap_ecmwf));
           end;
        end
        close(nc)
      end
