    ...
    do n=1,dimensio
        IF (Profile%posvar(n) == 1) THEN
            ...
            DO n2 = 1,dimensio
                IF (n2 .NE. n) THEN
                    nbout = nbout + 1
                    If ((Profile%posvar(n2) == 1).OR.(Profile%posvar(n2) == 2)) THEN
                        IF (Profile%interptab(n2) == 'x') THEN
                            normalsizes(2,n2)    = Agrif_Curgrid%nb(1)/agrif_Curgrid%spaceref(1)
                            normalsizes(1,nbout) = Agrif_Curgrid%nb(1)/agrif_Curgrid%spaceref(1)
                        ELSE IF (Profile%interptab(n2) == 'y') THEN
                            normalsizes(2,n2)    = Agrif_Curgrid%nb(2)/agrif_Curgrid%spaceref(2)
                            normalsizes(1,nbout) = Agrif_Curgrid%nb(2)/agrif_Curgrid%spaceref(2)
                        ELSE IF (Profile%interptab(n2) == 'z') THEN
                            normalsizes(2,n2)    = Agrif_Curgrid%nb(3)/agrif_Curgrid%spaceref(3)
                            normalsizes(1,nbout) = Agrif_Curgrid%nb(3)/agrif_Curgrid%spaceref(3)
                        ENDIF
                    ELSE
...
FUNCTION Agrif_Search_Flux(fluxname)
    ...
    IF (.NOT.foundflux) THEN
        write(*,*) 'The array flux '//TRIM(fluxname)//' has not been declared'
        stop
    ENDIF
...
!            print *,'flux stocke fiun = ',j2,fluxtab(unitarray3(1),j2)
                    enddo
                    fluxtabvar%array2(1:1,j) = fluxtabvar%array2(1:1,j) + &
                        SUM(fluxtab(unitarray3(1):normalsizes3(1),j1:j1+Agrif_Curgrid%spaceref(2)))
            ...
            CASE(2)
                j1 = unitarray3(2)
                Do j=unitarray3(2),normalsizes3(2)
                    fluxtabvar%array2(2:2,j) = fluxtabvar%array2(2:2,j) + &
                        SUM(fluxtab(unitarray3(1):normalsizes3(1),j1:j1+Agrif_Curgrid%spaceref(2)))
                    j1 = j1+Agrif_Curgrid%spaceref(2)
                EndDo
            END SELECT
...
Subroutine Agrif_Save_Fluxtab_child(Flux,Fluxtab)
    ...
    do n=1,dimensio
        IF (Profile%posvar(n) == 1) THEN
            ...
            CASE(1)
            CASE(2)
                fluxtabvar%array2(1:1,unitarray2(2):normalsizes2(2)) = &
                    - fluxtab(unitarray3(1):normalsizes3(1),unitarray3(2):normalsizes3(2))
!        print *,'flux stocke = ',fluxtab(unitarray3(1):normalsizes3(1),
!     &                        unitarray3(2):normalsizes3(2))
            END SELECT

            unitarray3(n) = unitarray3(n) + Agrif_Curgrid%nb(n)/Agrif_Curgrid%spaceref(n)
            normalsizes3(n) = unitarray3(n)
          
            SELECT CASE(dimensio)
            CASE(1)
            CASE(2)
                fluxtabvar%array2(2:2,unitarray2(2):normalsizes2(2)) = &
                    - fluxtab(unitarray3(1):normalsizes3(1),unitarray3(2):normalsizes3(2))
            END SELECT
        ENDIF
    enddo
    ...
    do n=1,dimensio
        IF (Profile%posvar(n) == 1) THEN
            ...
            CASE(2)
                ...
                Loctab(2) = Agrif_Curgrid%ix(locind)+Agrif_Curgrid%nb(locind)/Agrif_Curgrid%spaceref(locind)
                j1 = agrif_curgrid%ix(2)
                j2 = agrif_curgrid%ix(2)+agrif_curgrid%nb(2)/Agrif_curgrid%spaceref(2)
