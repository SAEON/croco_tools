...
%  Update    Feb-2014 by Gildas Cambon: cleaning
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
zeta=ext_data_OGCM(nc,lon,lat,'ssh',tin,lon,lat,1,Roa,interp_method);

if ~isempty(nc_clm)
%
% Read and extrapole the 2D variables
%
  u2d=ext_data_OGCM(nc,lon,lat,'ubar',tin,lon,lat,1,Roa,interp_method);
  v2d=ext_data_OGCM(nc,lon,lat,'vbar',tin,lon,lat,1,Roa,interp_method);
%
...
% 
  ...
  for k=1:NZ
    ...
    temp(k,:,:)=ext_data_OGCM(nc,lon,lat,'temp',tin,lon,lat,k,Roa,interp_method);
    salt(k,:,:)=ext_data_OGCM(nc,lon,lat,'salt',tin,lon,lat,k,Roa,interp_method);
  end
end
%
...
% Read and extrapole the 2D variables
  if obc(1)==1
    ...
    ubar1d=ext_data_OGCM(nc,lon,lat,'ubar',tin,...
	   squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),1,Roa,interp_method);
    vbar1d=ext_data_OGCM(nc,lon,lat,'vbar',tin,...
	   squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),1,Roa,interp_method);
...
%
  for k=1:NZ
    ...
    if obc(1)==1 % Southern boundary
      t1d=squeeze(ext_data_OGCM(nc,lon,lat,'temp',tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method));
      s1d=squeeze(ext_data_OGCM(nc,lon,lat,'salt',tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method));
      temp_south(k,:)=squeeze(t1d(1,:));
      salt_south(k,:)=squeeze(s1d(1,:));
      u1d=ext_data_OGCM(nc,lon,lat,'u',tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method);
      v1d=ext_data_OGCM(nc,lon,lat,'v',tin,...
                  squeeze(lon(1:2,:)),squeeze(lat(1:2,:)),...
                  k,Roa,interp_method);
