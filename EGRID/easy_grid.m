 function [lon4,lat4,pn,pm,ang,lone,late] = easy_grid(nx,ny,size_x,size_y,tra_lon,tra_lat,rot);
 %
 %   Easy grid make an rectangular, orthogonal grid with minimal gridsize variation
...
%  y = (y2-y1)*[0:1:nw]/nw + y1;  
 ...
 %% Compute pn and pm
   pnu = gc_dist(lonu(:,1:end-1),latu(:,1:end-1),lonu(:,2:end),latu(:,2:end));
   pn = 0.*lon4;
   pn(:,2:end-1) = pnu;
   pn(:,1  ) = pn(:,2    );
   pn(:,end) = pn(:,end-1); 

   pmv = gc_dist(lonv(1:end-1,:),latv(1:end-1,:),lonv(2:end,:),latv(2:end,:));
   pm = 0.*lon4;
   pm(2:end-1,:) = pmv;
   pm(1  ,:) = pm(2    ,:);
   pm(end,:) = pm(end-1,:);
 ...
 %% Compute angles of local grid positive x-axis relative to east
   ...
   ang = lon4;
   ang_s = atan(dellat./(dellon+1e-14))*180/pi;
   ang_s(dellon<0 & dellat< 0) = ang_s(dellon<0 & dellat< 0) - 180;
   ang_s(dellon<0 & dellat>=0) = ang_s(dellon<0 & dellat>=0) + 180;
   ang_s(ang_s > 180) = ang_s(ang_s > 180) - 180;
   ang_s(ang_s <-180) = ang_s(ang_s <-180) + 180;
